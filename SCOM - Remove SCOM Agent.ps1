...
# Code Template V 1.1
# URL: https://au2mator.com/remove-scom-agent-scom-self-service-with-au2mator/
# Github: https://github.com/au2mator/SCOM-Remove-SCOM-Agent
#################
...
## Environment
[string]$OpsmgrServer = 'Demo01.au2mator.local'
[string]$LogPath = "C:\_SCOworkingDir\TFS\PS-Services\SCOM - Remove SCOM Agent"
[string]$LogfileName = "Remove SCOM Agent"
...
try {
    Write-au2matorLog -Type INFO -Text "Try to get Full Agent"
    $Agent = Get-SCOMAgent -DNSHostName "$c_SCOMAgent"

    if (($Agent)){
        Write-au2matorLog -Type INFO -Text "Full Agent found"
        try {
            Uninstall-SCOMAgent -Agent $Agent -Confirm:$false
            
            Write-au2matorLog -Type INFO -Text "SCOM Agent removed"
        }
        catch {
            Write-au2matorLog -Type ERROR -Text "Failed to remove SCOM Agent"
            $ErrorCount = 1
        }
    }
    else {
        Write-au2matorLog -Type INFO -Text "Full Agent not found"

        try {
            Write-au2matorLog -Type INFO -Text "Try to get agentless Agent"
            $Agent=Get-SCOMAgentlessManagedComputer -DNSHostName "$c_SCOMAgent"
            if (($Agent)){
                Write-au2matorLog -Type INFO -Text "Agentless Agent found"
                try {
                    Remove-SCOMAgentlessManagedComputer -Computer $Agent -Confirm:$false
                    Write-au2matorLog -Type INFO -Text "SCOM Agent removed"
                }
                catch {
                    Write-au2matorLog -Type ERROR -Text "Failed to remove SCOM Agent"
                ...
                Write-au2matorLog -Type INFO -Text "Agentless Agent not found"        
            }
        }
        catch{
            Write-au2matorLog -Type ERROR -Text "Error on trying to query agentless Agent"
            $ErrorCount = 1
        }
    }
    ...
    Write-au2matorLog -Type ERROR -Text "Error on trying to query Agent"
    $ErrorCount = 1
}
...
if ($ErrorCount -eq 0) {
    $au2matorReturn = "SCOM Maintenance Mode Schedule added"
    $AdditionalHTML = "
        "
    $Status = "COMPLETED"
}
else {
    $au2matorReturn = "failed to add Maintenance Mode Schedule, Error: $Error"
